...
const fetchData= async()=>{
  ...
  console.log("current user data in home component:", data);
  
    if (user) {
      console.log("User found in Home component:", user?.user?._id);
      setlogged(true);
      dispatch(login({user:data.user, isAuth:true}));
    }
...
useEffect(() => {
  ...
  useEffect(() => {
     if (user) {
      console.log("User found in Home component:", user?.user?._id);
      setlogged(true);
      console.log("nevid:",user)
      dispatch(login({user:user, isAuth:true}));
    }
    socket.on("connect", () => {
    ...
    return (
      <div className='home min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-6'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-slate-300'>Welcome back, {logged ? user?.firstName || 'User' : 'Guest'}</p>
        </div>
